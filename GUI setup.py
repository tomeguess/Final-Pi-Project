...
class Greenhouse(Frame):
    ...
    def layout(self):
        ...
        self.temp_disp = Label(self, text = f"Current Greenhouse Temperature \n\n\n ",
                             anchor = N, bg = self.COLOR,
                             height = self.HEIGHT, width = self.WIDTH,
                             font = (self.FONT, 20))
        # create grid manager for label
        self.temp_disp.grid(row = 0, column = 0, columnspan = 3,\
                          sticky = E+W+N+S)
    ...
    def update_labels(self):
        if self.serial_reader:
            self.temp_disp.config(text=f"Current Greenhouse Temperature \n\n\n{self.serial_reader.temp}")
            self.humidity_label.config(text=f"Current Greenhouse Humidity \n\n\n{self.serial_reader.humidity}")
            self.moisture_label.config(text=f"Current Soil Moisture \n\n\n{self.serial_reader.moisture}")
